...
def fill_feed_dict(img, lm, l, keep, is_train_value, images_placeholder, landmarks_placeholder, labels_placeholder, keep_prob, is_train):
    ...
    feed_dict = {
        ...
        landmarks_placeholder: landmark_feed,
...
def run_training(fold_num, train_tfrecord_path, test_tfrecord_path, train_batch_size=60, test_batch_size=30):
    with tf.Graph().as_default():
        ...
        images_batch, landmark_batch, label_batch = tf.train.shuffle_batch([images, landmark, label], batch_size=train_batch_size, capacity=2000,
                                                        min_after_dequeue=1000)

        images_test, landmark_test, label_test = read_and_decode_4_test(test_tfrecord_path)
        # 使用shuffle_batch可以随机打乱输入
        images_batch_test, landmark_batch_test, label_batch_test = tf.train.batch([images_test, landmark_test, label_test], batch_size=test_batch_size, capacity=2000)

        # Generate placeholders for the images and labels.
        images_placeholder, landmarks_placeholder, labels_placeholder, keep_prob, is_train = placeholder_inputs()

        # Build a Graph that computes predictions from the inference model.
        fp_logits, fe_logits = hyperface.inference(images_placeholder, keep_prob, is_train)

        # Add to the Graph the Ops for loss calculation.
        loss = hyperface.loss([fp_logits, fe_logits], labels_placeholder, landmarks_placeholder)

        # Add to the Graph the Ops that calculate and apply gradients.
        global_step = tf.Variable(0, trainable=False)
        train_op = hyperface.training(loss, flags.learning_rate, global_step)

        # Add the Op to compare the logits to the labels during evaluation.
        eval_correct = hyperface.evaluation(fe_logits, labels_placeholder)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True, gpu_options= gpu_options)) as sess:
            ...
            train_writer = tf.summary.FileWriter('./summaries_graph_1128_oulu_simplier_new12/'+str(fold_num)+'/train', sess.graph)
            test_writer = tf.summary.FileWriter('./summaries_graph_1128_oulu_simplier_new12/'+str(fold_num)+'/test', sess.graph)
            ...
            for step in range(flags.max_steps):
                ...
                if step % 50 == 0 or (step + 1) == flags.max_steps:
                    ...
                    if (step + 1) == flags.max_steps:
                        fp_logits_last_values = sess.run(fp_logits, feed_dict=test_feed_dict)
                        fp_logits_first_frame = fp_logits_last_values[0, :136]
                        lm_test_first_frame = lm_test[0, :136]
                        img_test_first_frame = img_test[0, :, :, 0]
                        np.savetxt('./summaries_graph_1128_oulu_simplier_new12/'+str(fold_num)+'/logit_lm.txt', fp_logits_first_frame)
                        np.savetxt('./summaries_graph_1128_oulu_simplier_new12/'+str(fold_num)+'/test_lm.txt', lm_test_first_frame)
                        np.savetxt('./summaries_graph_1128_oulu_simplier_new12/'+str(fold_num)+'/test_img.txt', img_test_first_frame)
