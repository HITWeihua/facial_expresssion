...
def inference(images, keep_prob, is_train):
    # conv1
    with tf.variable_scope('conv1'):  # dtan_tanspose 2 与 1 的不同
        ...
        conv13 = conv2d(conv1_tp, kernel3) + biases3

        add_layer1 = tf.add(conv12, tf.transpose(conv13, perm=[0, 1, 3, 2]))  # dtan_tanspose 3 与 2 的不同
