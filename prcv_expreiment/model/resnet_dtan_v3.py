...
def inference(images, keep_prob, is_train):
    ...
    with tf.variable_scope('block2'):
        ...
        layer_activation2 = res_block(layer_activation1, is_train, 64)

    # pool1
    with tf.variable_scope('pool1'):
        pool1 = max_pool_2x2(layer_activation2)  # 32*32

    # conv2
    with tf.variable_scope('block3'):
        layer_activation3 = res_block(pool1, is_train, 64)

        layer_activation4 = res_block(layer_activation3, is_train, 64)

    # pool2
    with tf.variable_scope('pool2'):
        pool2 = tf.nn.max_pool(layer_activation4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 16*16

    with tf.variable_scope('block4'):
        layer_activation5 = res_block(pool2, is_train, 64)

        layer_activation6 = res_block(layer_activation5, is_train, 64)

    # pool2
    with tf.variable_scope('pool3'):
        pool3 = tf.nn.max_pool(layer_activation6, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 16*16

    with tf.variable_scope('block5'):
        layer_activation7 = res_block(pool3, is_train, 64)

        layer_activation8 = res_block(layer_activation7, is_train, 64)

    # pool2
    with tf.variable_scope('pool4'):
        pool4 = tf.nn.max_pool(layer_activation8, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 16*16
