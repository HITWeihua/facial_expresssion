...
def block_top(landmarks, is_train):
    kernel1 = weight_variable([136, 32], stddev=0.1, name='weights', wd=0.01)
    biases1 = bias_variable([32], name='biases')
    landmarks = tf.reshape(landmarks, [-1, 136])
    fc = tf.nn.relu(tf.matmul(landmarks, kernel1) + biases1)
    return fc
