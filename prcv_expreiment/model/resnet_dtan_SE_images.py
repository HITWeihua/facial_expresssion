...
def Squeeze_excitation_layer(input_x, input_dim, out_dim, ratio, layer_name):
    with tf.variable_scope(layer_name):
        ...
        weights = weight_variable([112, 112], stddev=0.1, name='weights', wd=0.01)
        biases = bias_variable([112], name='biases')
        fc_1 = tf.nn.relu(tf.matmul(squeeze, weights)+biases)
        weights2 = weight_variable([112, 112], stddev=0.1, name='weights2', wd=0.01)
        biases2 = bias_variable([112], name='biases2')
        fc_2 = tf.nn.sigmoid(tf.matmul(fc_1, weights2)+biases2)
