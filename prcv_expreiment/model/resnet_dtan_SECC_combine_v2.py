...
def Squeeze_excitation_layer_combine_v2(input_x, input_dim, out_dim, ratio, layer_name):
    with tf.variable_scope(layer_name):
        squeeze1 = tf.nn.avg_pool(input_x, ksize=[1, input_dim, input_dim, 1], strides=[1, 2, 2, 1], padding='VALID')
        squeeze1 = tf.reshape(squeeze1, [-1, 64])

        first_dim = int(input_dim * input_dim / 4)

        squeeze2 = tf.reduce_mean(input_x, reduction_indices=[3], keep_dims=True)
        squeeze2 = tf.nn.avg_pool(squeeze2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
        squeeze2 = tf.reshape(squeeze2, [-1, first_dim])
        squeeze = tf.concat([squeeze1, squeeze2], -1)
        second_dim = int(len(squeeze[0]))


        weights = weight_variable([second_dim, 64], stddev=0.1, name='weights', wd=0.01)
        # biases = bias_variable([64], name='biases')
        fc_1 = tf.nn.relu(tf.matmul(squeeze, weights))
        weights2 = weight_variable([64, second_dim], stddev=0.1, name='weights', wd=0.01)
