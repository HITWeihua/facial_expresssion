...
# from prcv_expreiment.model import dtgn_rnn_ck as model
from prcv_expreiment.model import dtgn_td_rnn as model
...
def placeholder_inputs():
    ...
    landmarks_placeholder = tf.placeholder(tf.float32, shape=[None, model.OULU_SIMPLE_NUM, model.OULU_LANDMARKS_LENGTH/7])
...
def read_and_decode(filename):
    ...
    landmark = tf.reshape(landmark, [model.OULU_SIMPLE_NUM, -1])
            ...
            for step in range(flags.max_steps):
                ...
                if step % 100 == 0 or (step + 1) == flags.max_steps:
                    ...
                    if (step + 1) == flags.max_steps:
                        ...
                        np.savetxt(
                            '/home/duheran/facial_expresssion/prcv_expreiment/trainging/logits_output/dtgn_td_rnn/' + str(
                                fold_num) + '/logit.txt',
                        ...
                        np.savetxt(
                            '/home/duheran/facial_expresssion/prcv_expreiment/trainging/labels_output/dtgn_td_rnn/' + str(
                                fold_num) + '/test_l.txt',
                            l_test)
                        # fe_logits_last_values = sess.run(fe_logits, feed_dict=test_feed_dict)
                        # np.savetxt('./summaries/summaries_graph_1219/' + str(fold_num) + '/logit.txt',
