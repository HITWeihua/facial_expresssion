...
from prcv_expreiment.model import resnet_dtan_v2_centerloss as model
...
def placeholder_inputs():
    ...
    labels_placeholder = tf.placeholder(tf.float32, shape=(None, model.OULU_NUM_CLASSES))
...
def run_training(fold_num, train_tfrecord_path, test_tfrecord_path, train_batch_size=60, test_batch_size=30):
    with tf.Graph().as_default():
        ...
        fe_logits, feature = model.inference(images_placeholder, keep_prob, is_train)

        # Add to the Graph the Ops for loss calculation.
        loss, centers_update_op = model.loss(fe_logits, labels_placeholder, feature)

        # Add to the Graph the Ops that calculate and apply gradients.
        global_step = tf.Variable(0, trainable=False)
        train_op = model.training(loss, flags.learning_rate, global_step, centers_update_op)
