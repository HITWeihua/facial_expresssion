...
def placeholder_inputs():
    ...
    landmarks_placeholder = tf.placeholder(tf.float32, shape=[None, model.OULU_LANDMARK_LENGTH])
...
def run_training(fold_num, train_tfrecord_path, test_tfrecord_path, train_batch_size=60, test_batch_size=30):
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True, gpu_options= gpu_options)) as sess:
            ...
            train_writer = tf.summary.FileWriter('./summaries_new/summaries_graph_0419/'+str(fold_num)+'/train', sess.graph)
            test_writer = tf.summary.FileWriter('./summaries_new/summaries_graph_0419/'+str(fold_num)+'/test', sess.graph)
