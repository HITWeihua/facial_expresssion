...
def inference(images, keep_prob, is_train):
    images = tf.reshape(images,[-1,6,68,2])
    with tf.variable_scope('block_top'):
        ...
        # frames_features1 = block_top(images[:,2,:,:],is_train)
        # frames_features2 = block_top(images[:,5,:,:],is_train)
...
def placeholder_inputs():
    ...
    landmarks_placeholder = tf.placeholder(tf.float32, shape=[None, CK_LANDMARKS_LENGTH])
    labels_placeholder = tf.placeholder(tf.int32, shape=(None, CK_NUM_CLASSES))
...
def read_and_decode(filename):
    ...
    features = tf.parse_single_example(serialized_example,
                                       features={
                                           'label': tf.FixedLenFeature([CK_NUM_CLASSES], tf.float32),
    ...
    landmark = tf.slice(img, [IMAGE_PIXELS*CK_SIMPLE_NUM], [CK_LANDMARKS_LENGTH])
...
def run_training(fold_num, train_tfrecord_path, test_tfrecord_path, train_batch_size=60, test_batch_size=30):
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True, gpu_options= gpu_options)) as sess:
            ...
            train_writer = tf.summary.FileWriter('./summaries_new2/summaries_graph_0420/'+str(fold_num)+'/train', sess.graph)
            test_writer = tf.summary.FileWriter('./summaries_new2/summaries_graph_0420/'+str(fold_num)+'/test', sess.graph)
