...
def inference(images, keep_prob, is_train):
    with tf.variable_scope('ds0'):
        ...
        variable_summaries(ds0_activation, 'ds0_activation')
        # variable_summaries(conv1_dropout, 'conv1_dropout')

    ds1_input = tf.concat([images, ds0_activation], 3)
    with tf.variable_scope('ds1'):
        kernel = weight_variable([5, 5, SIMPLE_NUM+64, 64], stddev=0.1, name='weights', wd=0.01)
        biases = bias_variable([64], name='biases')
        ds1 = conv2d(ds1_input, kernel) + biases
        ds1_bn = batch_norm(ds1, 64, is_train)
        ds1_activation = ACTIVATION(ds1_bn, name='activate')  # 32*32
        # conv2_dropout = tf.nn.dropout(conv2_activation, keep_prob=1.0)
        variable_summaries(ds1_activation, 'ds1_activation')

    ds2_input = tf.concat([images, ds0_activation, ds1_activation], 3)
    with tf.variable_scope('ds2'):
        kernel = weight_variable([5, 5, SIMPLE_NUM + 64*2, 64], stddev=0.1, name='weights', wd=0.01)
        biases = bias_variable([64], name='biases')
        ds2 = conv2d(ds2_input, kernel) + biases
        ds2_bn = batch_norm(ds2, 64, is_train)
        ds2_activation = ACTIVATION(ds2_bn, name='activate')  # 32*32
        # conv2_dropout = tf.nn.dropout(conv2_activation, keep_prob=1.0)
        variable_summaries(ds2_activation, 'ds2_activation')

    with tf.variable_scope('trans1'):
        kernel = weight_variable([1, 1, 64, 16], stddev=0.1, name='weights', wd=0.01)
        biases = bias_variable([16], name='biases')
        trans1 = conv2d(ds2_activation, kernel) + biases
        trans1_bn = batch_norm(trans1, 16, is_train)
        trans1_activation = ACTIVATION(trans1_bn, name='activate')  # 32*32
        # conv2_dropout = tf.nn.dropout(conv2_activation, keep_prob=1.0)
        trans1_pool = max_pool_2x2(trans1_activation)
        variable_summaries(trans1_pool, 'trans1_activation')
        trans1_pool_flat = tf.reshape(trans1_pool, [-1, 32 * 32 * 16])

    # fc3 facial expression
    with tf.variable_scope('fc1'):
        weights = weight_variable([32*32*16, NUM_CLASSES], stddev=0.1, name='weights', wd=0.01)
        biases = bias_variable([NUM_CLASSES], name='biases')
        logits = tf.matmul(trans1_pool_flat, weights) + biases
