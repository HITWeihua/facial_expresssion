...
def block_top(image, is_train):
    ...
    with tf.variable_scope('pool4'):
        pool4 = max_pool_2x2(layer_activation3)  # 4*4

    with tf.variable_scope('block4'):
        layer_activation4 = res_block(pool4, is_train, 16)  # 4*4
    return layer_activation4
...
def inference(images, keep_prob, is_train):
    with tf.variable_scope('block_top'):
        for i in range(SIMPLE_NUM):
            if i == 0:
                ...
                frames_features_concat = frames_features
            else:
                frames_features = block_top(images[:, :, :, i], is_train)
                frames_features_concat = tf.concat([frames_features_concat, frames_features], axis=-1)
    ...
    h_pool4_flat = tf.reshape(frames_features_concat, [-1, 4 * 4 * 112])
