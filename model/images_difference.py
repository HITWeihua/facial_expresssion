...
def calculate_differ(images):
    for i in range(SIMPLE_NUM):
        if i == 0:
            frames = tf.reshape(images[:, :, :, i], [-1, 64, 64, 1])
            old_frames = frames
        elif i == 1:
            frames = tf.reshape(images[:, :, :, i], [-1, 64, 64, 1])
            frames_concat = frames - old_frames
            old_frames = frames
        else:
            frames = tf.reshape(images[:, :, :, i], [-1, 64, 64, 1])
            frames_concat = tf.concat([frames_concat, frames - old_frames], axis=-1)
            old_frames= frames
    return frames_concat


def inference(images, keep_prob, is_train):
    image_differ = calculate_differ(images)

    # conv1
    with tf.variable_scope('block1'):
        kernel1 = weight_variable([5, 5, SIMPLE_NUM-1, 64], stddev=0.1, name='weights', wd=0.0)
        biases1 = bias_variable([64], name='biases')
        conv1 = conv2d(image_differ, kernel1) + biases1
