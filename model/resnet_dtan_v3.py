...
def inference(images, keep_prob, is_train):
    ...
    with tf.variable_scope('block1'):
        kernel1 = weight_variable([5, 5, SIMPLE_NUM, 64], stddev=0.1, name='weights', wd=0.0)
    ...
    with tf.variable_scope('conv9'):
        kernel9 = weight_variable([4, 4, 64, 64], stddev=0.1, name='weights', wd=0.0)
        biases9 = bias_variable([64], name='biases')
        conv9 = tf.nn.conv2d(pool4, kernel9, strides=[1, 1, 1, 1], padding='VALID') + biases9
    # fc1
    ...
    h_conv9_flat = tf.reshape(conv9, [-1, 1 * 1 * 64])
    with tf.variable_scope('fc3_ep'):
        weights = weight_variable([64, NUM_CLASSES], stddev=0.1, name='weights', wd=0.01)
        biases = bias_variable([NUM_CLASSES], name='biases')
        fe_logits = tf.matmul(h_conv9_flat, weights) + biases
