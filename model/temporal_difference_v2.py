...
def block_top(image, is_train):
    ...
    with tf.variable_scope("block1"):
        layer_activation = res_block(pool1, is_train, 16)  # 32*32
...
def inference(images, keep_prob, is_train):
    ...
    with tf.variable_scope('block1'):
        ...
        conv1_activation = ACTIVATION(conv1_bn, name='activate')  # 32*32
    ...
    with tf.variable_scope('block2'):
        ...
        add_layer1_activation = ACTIVATION(add_layer1_bn, name='activate')  # 32*32
        variable_summaries(add_layer1_activation)

    # pool1
    with tf.variable_scope('pool1'):
        pool1 = max_pool_2x2(add_layer1_activation)  # 16*16
    ...
    with tf.variable_scope('block3'):
        ...
        conv4_activation = ACTIVATION(conv4_bn, name='activate')  # 16*16
        ...
        add_layer2_activation = ACTIVATION(add_layer2_bn, name='activate')  # 16*16
        variable_summaries(add_layer2_activation)

    # pool2
    with tf.variable_scope('pool2'):
        pool2 = tf.nn.max_pool(add_layer2_activation, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    ...
    with tf.variable_scope('block4'):
        ...
        conv6_activation = ACTIVATION(conv6_bn, name='activate')
        ...
        add_layer3_activation = ACTIVATION(add_layer3_bn, name='activate')
        variable_summaries(add_layer3_activation)

    # pool2
    with tf.variable_scope('pool3'):
        pool3 = tf.nn.max_pool(add_layer3_activation, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
