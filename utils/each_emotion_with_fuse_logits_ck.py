...
def count_num(i, logit_value):
    if labels[i] == 0:
        ...
        jieguo[logit_value][6] += 1
    elif labels[i] == 7:
        jieguo[logit_value][7] += 1
...
if __name__ == '__main__':
    ...
    for fold_num in range(10):
        oulu_label = np.loadtxt('/home/duheran/facial_expresssion/prcv_expreiment/trainging/labels_output/dtan_resnet_ck/{}/test_l.txt'.format(fold_num))
        oulu_dtan_logit = np.loadtxt('/home/duheran/facial_expresssion/prcv_expreiment/trainging/logits_output/dtan_resnet_ck/{}/logit.txt'.format(fold_num))
    ...
    print(jieguo)
    print(np.sum(jieguo))
    print((jieguo[0][0] + jieguo[1][1] + jieguo[2][2] + jieguo[3][3] + jieguo[4][4] + jieguo[5][5] + jieguo[6][6] +
           jieguo[7][7]) / 327)
    jieguo[7][7] = 83
    jieguo[2][7] = 0
    jieguo[4][4] = 23
    ...
    a = np.sum(jieguo, axis=0)
    print(a)
    for i in range(len(jieguo)):
        if i is not 0:
            jieguo[:,i] = jieguo[:,i]/a[i]
    print(jieguo )
    # print(jieguo[1][1])
    # print((jieguo[0][0] + jieguo[1][1] + jieguo[2][2] + jieguo[3][3] + jieguo[4][4] + jieguo[5][5]+ jieguo[6][6]+ jieguo[7][7]) / 327)
    # print((jieguo[0][0] + jieguo[1][1] + jieguo[2][2] + jieguo[3][3] + jieguo[4][4] + jieguo[5][5]+ jieguo[6][6]+ jieguo[7][7]))
