...
def write_2_image(image_names_path, landmarks_names_path, image_names, is_flipped=False, angle=0):
    express_array = np.zeros((64, 64, 14))
    index = 0
    for image_name in image_names:
        image_path = os.path.join(image_names_path, image_name)
        lm_path = os.path.join(landmarks_names_path, image_name)
        img = Image.open(image_path)
        lm_pic = Image.open(lm_path)
        lm_pic
        # img = img.convert("L")
        if is_flipped:
...
image_base_path = "/home/duheran/facial_expression/oulu_sampling/"
landmark_scale = "/home/duheran/facial_expression/oulu_img_gama0.2"
...
if __name__ == '__main__':
    ...
    for tf_num in range(10):
        ...
        writer = tf.python_io.TFRecordWriter("/home/duheran/facial_expression/oulu_imglm/{}/oulu_joint_test_{}.tfrecords".format(str(tf_num), str(tf_num)))  # 要生成的文件

        # with open("./data_pairs/landmark/{}/test_subjects.txt".format(str(tf_num)), 'r') as f:
        #     subject_list = f.readlines()
        img_people_list = [x for x in os.listdir(image_base_path) if str(tf_num) in x[3]]
        lm_people_list = [x for x in os.listdir(landmark_scale) if str(tf_num) in x[3]]
        print(img_people_list)
        for people in img_people_list:
            # landmarks_people_dir_path = os.path.join(landmarks_base_path, people)
            people_img_dir_path = os.path.join(image_base_path, people)
            people_lm_dir_path = os.path.join(landmark_scale, people)

            # lable_path_people = os.path.join(lable_base_path, people)
            express_list = [x for x in os.listdir(people_img_dir_path) if '.DS' not in x]

            for num in range(len(express_list)):
                landmarks_names_path = os.path.join(people_lm_dir_path, express_list[num])
                image_names_path = os.path.join(people_img_dir_path, express_list[num])
                ...
                landmark_names = [x for x in os.listdir(landmarks_names_path) if x in image_names]

                lable_value = int(num)
                lable_vec = np.zeros((6))
                lable_vec[lable_value] = 1
                ...
                # for angle in angles:
                assert len(landmark_names)+len(image_names) == 14, "files number not equal to 14. files: {}".format(image_names_path)

                # landmarks_vec = preprocess_data(lable_vec, image_names_path, landmark_names, is_flipped=False, add_noise=False, rotation=0)
                images_vec = write_2_image(image_names_path, landmarks_names_path, image_names, is_flipped=False, angle=0)
