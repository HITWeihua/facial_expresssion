...
def read_and_decode(filename):
    ...
    images1 = tf.slice(img, [0], [64 * 64 * 1])
    images1 = tf.reshape(images1, [64, 64, 1])

    images2 = tf.slice(img, [64*64*6], [64*64*2])
    images2 = tf.reshape(images2, [64, 64, 2])
...
def run_training(fold_num, train_tfrecord_path, test_tfrecord_path, train_batch_size=60, test_batch_size=30):
    with tf.Graph().as_default():
        ...
        fe_logits = mhi.inference(images_placeholder, keep_prob, is_train)

        # Add to the Graph the Ops for loss calculation.
        loss = mhi.loss(fe_logits, labels_placeholder)

        # Add to the Graph the Ops that calculate and apply gradients.
        global_step = tf.Variable(0, trainable=False)
        train_op = mhi.training(loss, flags.learning_rate, global_step)

        # Add the Op to compare the logits to the labels during evaluation.
        eval_correct = mhi.evaluation(fe_logits, labels_placeholder)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True, gpu_options= gpu_options)) as sess:
            ...
            train_writer = tf.summary.FileWriter('./summaries_graph_1207/'+str(fold_num)+'/train', sess.graph)
            test_writer = tf.summary.FileWriter('./summaries_graph_1207/'+str(fold_num)+'/test', sess.graph)
